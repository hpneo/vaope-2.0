...
**/
	...
	function fotos($fotos){
		if($fotos){
			echo '
			<h5>Fotos:</h5>
			<ul class="flotante" id="galeria">';
			foreach($fotos as $foto){
				echo embeber_foto($foto->ruta);
			}
			echo '</ul>';
		}
	}
	...
	function embeber_foto($ruta){
		if(ereg('\.[a-z]{3}$', $ruta))
			...
			if(strstr($ruta, 'www.flickr.com') || strstr($ruta, 'flickr.com')){
				$_ruta = ereg_replace('^(.*)flickr\.com\/photos\/', '', $ruta);
				$id = explode('/', $_ruta);
				if($id[1]=="sets"){
					$xml = simplexml_load_file('http://api.flickr.com/services/rest/?method=flickr.photosets.getPhotos&api_key=7072bee5676a38d5038f087c69237cae&per_page=5&photoset_id='.$id[2]);
					foreach($xml->photoset->photo as $foto){
						$fotos.= '
						<li>
							<a href="http://farm'.$foto->attributes()->farm.'.static.flickr.com/'.$foto->attributes()->server.'/'.$foto->attributes()->id.'_'.$foto->attributes()->secret.'.jpg" title="'.$foto->attributes()->title.'">
								<img src="http://farm'.$foto->attributes()->farm.'.static.flickr.com/'.$foto->attributes()->server.'/'.$foto->attributes()->id.'_'.$foto->attributes()->secret.'_s.jpg" />
							</a>
						</li>
						';
					}
					...
					$xml = simplexml_load_file('http://api.flickr.com/services/rest/?method=flickr.photos.getInfo&api_key=7072bee5676a38d5038f087c69237cae&photo_id='.$id[1]);
					return '
					<li>
						<a href="http://farm'.$xml->photo->attributes()->farm.'.static.flickr.com/'.$xml->photo->attributes()->server.'/'.$xml->photo->attributes()->id.'_'.$xml->photo->attributes()->secret.'.jpg" title="'.$xml->photo->attributes()->title.'">
			...
			elseif(strstr($ruta, 'www.zooomr.com') || strstr($ruta, 'zooomr.com')){
				$_ruta = ereg_replace('^(.*)zooomr\.com\/photos\/', '', $ruta);
				$id = explode('/', $_ruta);
				if($id[1]=="sets"){
					$xml = simplexml_load_file('http://api.zooomr.com/services/rest/?api_key=527fdb94db431d4e64b500c2511fa20c&method=zooomr.photosets.getPhotos&per_page=5&photoset_id='.$id[2]);
					foreach($xml->photoset->photo as $foto){
						if($foto->attributes()->isprimary=='1')
							continue;
						$fotos.= '
						<li>
							<a href="http://static.zooomr.com/images/'.$foto->attributes()->id.'_'.$foto->attributes()->secret.'.jpg" title="'.$foto->attributes()->title.'">
								<img src="http://static.zooomr.com/images/'.$foto->attributes()->id.'_'.$foto->attributes()->secret.'_s.jpg" />
							</a>
						</li>
						';
					}
					...
					$xml = simplexml_load_file('http://api.zooomr.com/services/rest/?api_key=527fdb94db431d4e64b500c2511fa20c&method=zooomr.photos.getInfo&photo_id='.$id[1]);
					return '
					<li>
						<a href="http://static.zooomr.com/images/'.$xml->photo->attributes()->id.'_'.$xml->photo->attributes()->secret.'.jpg" title="'.$xml->photo->title.'">
							<img src="http://static.zooomr.com/images/'.$xml->photo->attributes()->id.'_'.$xml->photo->attributes()->secret.'_s.jpg" />
						</a>
					</li>
					';
				}
	...
	function imprimir_js(){
		...
		if(estas_en('local')){
			echo'
			<script src="'.url.'/plantillas/js/lightbox/jquery.lightbox-0.5.pack.js" type="text/javascript"></script> 
			<link href="'.url.'/plantillas/js/lightbox/jquery.lightbox-0.5.css" media="screen" rel="stylesheet" type="text/css"/>
			<link type="text/css" rel="stylesheet" href="'.url.'/plantillas/js/rating/style.css" />
			<script type="text/javascript" src="'.url.'/plantillas/js/rating/jquery.rater.min.js"></script>
			<script type="text/javascript">
				...
				$(function(){
			        $(\'#galeria a\').lightBox({
			        	imageLoading: \''.url.'/plantillas/js/lightbox/images/loading.gif\',
						imageBtnClose: \''.url.'/plantillas/js/lightbox/images/close.gif\',
						imageBtnPrev: \''.url.'/plantillas/js/lightbox/images/prev.gif\',
						imageBtnNext: \''.url.'/plantillas/js/lightbox/images/next.gif\',
						txtImage: \'Foto\',
						txtOf: \'de\'
			        });
			    });
